# User-space, Statically Defined Tracing (USDT) for Bitkincoin Core

Bitkincoin Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Bitkincoin Core
...
Multiple tools can list the available tracepoints in a `bitkincoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bitkincoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bitkincoind
stap net        outbound_message 0x0000000000107c05 /src/bitkincoind
stap validation block_connected  0x00000000002fb10c /src/bitkincoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bitkincoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitkincoind | grep NT_STAPSDT -A 4 -B 2
