...
Bitkincoin Core version 0.20.0 is now available from:

  <https://bitkincoin.com/bin/bitkincoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Bitkincoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bitkincoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Bitkincoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bitkincoin Core. Bitkincoin Core logs debug messages
...
- OpenSSL is no longer used by Bitkincoin Core.  (#17265)

- BIP70 support has been fully removed from Bitkincoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitkin/bitkin/issues/16599),
  [PR #16702](https://github.com/bitkin/bitkin/pull/16702), and the
  `bitkincoind help` for more information.  This option is experimental and
...
- All custom settings configured when Bitkincoin Core starts are now
...
- The "Start Bitkincoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitkincoin.com/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Bitkincoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bitkincoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitkincoin-core/HWI) for
  signing.  Future versions of Bitkincoin Core should support a GUI option
...
- Bitkincoin Core's automatically-generated source code documentation is
  now available at https://doxygen.bitkincoin.com.  (#17596)
...
- The `bitkincoin-cli` utility used with the `-getinfo` parameter now
...
- The `bitkincoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitkincoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitkincoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bitkincoin` binary (hebasto)
...
- #18702 Fix ASLR for bitkincoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitkincoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitkincoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitkincoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitkincoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitkincoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bitkincoin Core files (hebasto)
...
- #17119 Fix broken bitkincoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitkincoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitkincoin.com (laanwj)
...
- #17648 Rename wallet-tool references to bitkincoin-wallet (hel-o)
...
- #18341 Replace remaining literal BTK with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitkincoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitkincoin-cli -getinfo (malevolent)
