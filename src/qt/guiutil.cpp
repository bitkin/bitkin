...
QString formatBitkincoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitkincoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitkincoinUnits::format(BitkincoinUnits::BTK, info.amount, false, BitkincoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("bitkincoin-%s.desktop", chain);
}
