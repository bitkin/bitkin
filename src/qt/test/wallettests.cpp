...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_bitkincoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_bitkincoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_bitkincoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = BitkincoinUnits::formatWithUnit(unit, balance, false, BitkincoinUnits::SeparatorStyle::ALWAYS);
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("bitkincoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_bitkincoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
