...
export P_CI_DIR="${BASE_BUILD_DIR}/bitkincoin-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/bitkincoin-config.h > src/config/bitkincoin-config.h.tmp && mv src/config/bitkincoin-config.h.tmp src/config/bitkincoin-config.h'
fi
