...
"""Script for verifying Bitkincoin Core release binaries
...
def main(args):
    ...
    if not success:
        print("bitkincoin.org failed to provide signature file, "
              "but bitkincoincore.org did?")
        print("wget output:")
        print(indent(output, '\t'))
    ...
    if not files_are_equal(sigfile1, sigfile2):
        print("bitkincoin.org and bitkincoincore.org signature files were not equal?")
    ...
    if retval != 0:
        if retval == 1:
            ...
            print("gpg error. Do you have the Bitkincoin Core binary release "
