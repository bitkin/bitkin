...
###   bitkincoin.com and bitkincoin.org and compares them.
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitkincoin.org failed to provide signature file, but bitkincoin.com did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitkincoin.org and bitkincoin.com signature files were not equal?"
